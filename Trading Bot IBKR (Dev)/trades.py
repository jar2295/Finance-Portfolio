...
class Trade():
    ...
    def new_trade(self, trade_id: str, order_type: str, side: str, enter_or_exit: str, price: float = 0.00, stop_limit_price: float =0.00) -> dict:
     ...
     #adding a specific instrument to a particualr order Leg ID       
    ...
    # make it a good till cancel order   
    ...
    #make the order an opisite side one
    def modify_side(self, side:Optional[str], order_leg_id: int = 0) -> None:
        if side and side not in ['buy', 'sell', 'sell_short', 'buy_to_cover']:
            raise ValueError("you Passes through an invalid side.")
        
        if side:
            self.side['orderLegCollection']['order_leg_id']['instructions'] = side.upper()
        else:
            self.order['orderLegCollection'][order_leg_id]['instrctions'] = self.order_instructions[self.enter_or_exit][self.side_opposite]
    ...
    def _convert_to_trigger(self):
        ...
        if self.order and self.triggered_added ==False:
            self.order['orderStrategyType'] == 'TRIGGER'
            self.order['childOrderStrategies'] = []
            self.triggered_added = True
    ...
    def add_leg(self, order_leg_id: int, symbol: str, quantity: int, asset_type:str, sub_asset_type: str = None) -> List[Dict]:
        #define the leg
        leg = {}
        leg['instrumnet']['symbol'] = symbol
        leg['instrumnet']['assetType'] = asset_type
        leg['quantity']= quantity
        
        if sub_asset_type:
            leg['instrumnet']['subAssetType'] = sub_asset_type
